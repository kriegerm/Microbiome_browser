...
sample_metadata <- sample_data(phy_obj_oscc)
...
sample_data(phy_obj_oscc) <- sample_metadata
```
...
length(unique(taxa_names(phy_obj_oscc)))
new_otu_names <- sprintf("ASV%04d", seq_along(taxa_names(phy_obj_oscc)))
length(unique(new_otu_names)) 

# Assign the new names to the phyloseq object
taxa_names(phy_obj_oscc) <- new_otu_names

# Verify the change
head(taxa_names(phy_obj_oscc))
```

### 2.4.2 Clean up tax table 
```{r}
phy_obj_oscc <- tax_fix(phy_obj_oscc)

tax_table_edit <- phy_obj_oscc@tax_table %>% as.data.frame()
...
```{r}
# Filter singletons
phy_obj_oscc_fs <- prune_taxa(taxa_sums(phy_obj_oscc) > 1, phy_obj_oscc)
length(unique(taxa_names(phy_obj_oscc_fs)))
```

### 2.4.4 Histogram of counts
```{r}
otu_table <- otu_table(phy_obj_oscc_fs)
...
# 3.0 Save
```{r}
saveRDS(list(phy_obj_oscc_fs=phy_obj_oscc_fs, 
             phy_obj_oscc= phy_obj_oscc), 
